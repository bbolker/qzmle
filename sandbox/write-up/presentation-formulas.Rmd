...
Find the model parameter that maximize the the likelihood, or $ \hat \theta = \argmax_\theta l(\theta)$ in which the value of $\hat \theta$ satisfies:

$$\frac{\partial l}{\partial \theta} = 0 $$  

## Slide 3

Basic: $ X \sim f(\theta_i)$ 

e.g. $X \sim \textrm{Binom}(p)$ or $X \sim \textrm{Gamma}(\alpha, \beta)$

\bigskip
\bigskip

Parameter $\theta$ depends on covariates $w_i$: 
$$X \sim f(\theta) \;\;\;\;\; \theta = h(w_1,...,w_i)$$ 
